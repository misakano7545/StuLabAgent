...
3. WMI `Win32_IP4RouteTable`（默认路由的 `InterfaceIndex` 先对齐 `NICConfiguration`，再用其 `Index` 查 `NetworkAdapter` 得 `NetConnectionID`；Win7 上路由 ifIndex 常与 `NetworkAdapter.InterfaceIndex` 不一致）
